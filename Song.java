...
public class Song extends AudioContent implements Comparable<Song>// implement the Comparable interface
{
			...
			String composer, Song.Genre genre, String lyrics)
	{
		...
		super(title, year, id, type, audioFile, length);	// using superclass AudioContent
		this.artist = artist; 
	...
	{
		super.printInfo(); // superclass from AudioContent class to get the title, id, type, audiofile and length
	...
	public void play()
	{
		this.setAudioFile(this.lyrics);	// setting the audiofile to the lyrics given 
		super.play(); // super class play() from the AudioContent class
	...
	{
		Song oth = (Song) other; // creating Song object using other
		return (super.equals(oth) && this.artist.equals(oth.artist) && this.composer.equals(oth.composer));		
		// ^ using the audioContent method equals() by using superclass and using just regular equals()
	...
	public int compareTo(Song other)
	{
			...
			// it returns either 1, -1, or 0
			// 1 meaning this title is higher alphabetically than other title
			// -1 meaning lower alphabetically
			// 0 meaning the same(not likely)
	}
}
